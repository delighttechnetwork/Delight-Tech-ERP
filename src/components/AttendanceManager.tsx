...
export default function AttendanceManager({ role, teacherDetails }: AttendanceProps) {
  ...
  // 2. Fetch students and existing attendance for selected class & date (Teachers only)
  useEffect(() => {
    ...
    try {
      ...
      const res = await fetch('/api/attendance', {
        ...
        body: JSON.stringify({
          ...
          date: selectedDate,
          records,
          teacherName: teacherDetails?.fullname || 'Class Tutor'
        })
      });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200/80 pb-5">
        ...
        {role === 'principal' && (
          <button
            ...
            className="flex items-center gap-2 px-3.5 py-2.5 border border-slate-200 hover:bg-slate-50 text-slate-705 bg-white rounded-xl text-xs font-bold transition shrink-0"
          >
          ...
          <div className="bg-white p-5 rounded-3xl border border-slate-200/80 shadow-xs flex flex-col md:flex-row md:items-center gap-4">
            <div className="space-y-1">
              ...
              <select
                ...
                className="bg-slate-50 border border-slate-150 text-xs px-3.5 py-2 rounded-xl outline-none font-semibold text-slate-700"
              >
                {classes.map(c => (
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block font-mono font-mono">Roll-call Date</label>
              <input
                type="date"
                value={selectedDate}
                onChange={(e) => setSelectedDate(e.target.value)}
                className="bg-slate-50 border border-slate-150 text-xs px-3.5 py-2 rounded-xl outline-none text-slate-700"
              />
            </div>

            {teacherDetails?.assigned_class && selectedClass !== teacherDetails.assigned_class && (
              <div className="bg-slate-50 p-2 border border-slate-200 rounded-xl flex items-center gap-1.5 text-[10px] text-slate-400 font-medium">
